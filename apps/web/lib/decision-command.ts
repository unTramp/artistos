...
export async function resolveDecisionMutationContext(request: Request): Promise<ArtistMutationContext | Response> {
  ...
  if (!Number.isInteger(expectedVersion) || expectedVersion < 1) {
    return apiErrorResponse({ code: "INVALID_IF_MATCH", message: "If-Match must contain a positive integer Decision version." }, resolution.traceId, 400);
  }
