...
export async function resolveLearningMutationContext(request: Request): Promise<ArtistMutationContext | Response> {
  ...
  if (!Number.isInteger(expectedVersion) || expectedVersion < 1) {
    return apiErrorResponse({ code: "INVALID_IF_MATCH", message: "If-Match must contain a positive integer Learning version." }, resolution.traceId, 400);
  }
