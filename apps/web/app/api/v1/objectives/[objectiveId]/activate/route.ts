import { ActivatePlanningObjectiveService } from "@artist-os/core";
...
export async function POST(request: Request, context: { params: Promise<{ objectiveId: string }> }) {
  ...
  const result = await new ActivatePlanningObjectiveService(new PgPlanningObjectiveWriter(getDatabaseRuntime().db))
