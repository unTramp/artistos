import { CompletePlanningObjectiveService } from "@artist-os/core";
...
export async function POST(request: Request, context: { params: Promise<{ objectiveId: string }> }) {
  ...
  const result = await new CompletePlanningObjectiveService(new PgPlanningObjectiveWriter(getDatabaseRuntime().db))
