import { ExpireOperationalActionService } from "@artist-os/core";
...
export async function POST(request: Request, context: { params: Promise<{ actionId: string }> }) {
  ...
  const result = await new ExpireOperationalActionService(new PgOperationalActionWriter(getDatabaseRuntime().db))
    .execute({ actionId, ...(body.reason ? { reason: body.reason } : {}) }, resolution.commandContext);
