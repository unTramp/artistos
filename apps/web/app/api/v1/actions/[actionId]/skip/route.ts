import { SkipOperationalActionService } from "@artist-os/core";
...
export async function POST(request: Request, context: { params: Promise<{ actionId: string }> }) {
  ...
  const result = await new SkipOperationalActionService(new PgOperationalActionWriter(getDatabaseRuntime().db))
