import { ReopenOperationalActionService } from "@artist-os/core";
...
export async function POST(request: Request, context: { params: Promise<{ actionId: string }> }) {
  ...
  const result = await new ReopenOperationalActionService(new PgOperationalActionWriter(getDatabaseRuntime().db))
