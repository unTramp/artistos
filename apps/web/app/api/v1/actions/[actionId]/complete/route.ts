import { CompleteOperationalActionService } from "@artist-os/core";
...
export async function POST(request: Request, context: { params: Promise<{ actionId: string }> }) {
  ...
  const body = await readJson<{ evidenceRef?: string }>(request, resolution.traceId);
  if (body instanceof Response) return body;
  const result = await new CompleteOperationalActionService(new PgOperationalActionWriter(getDatabaseRuntime().db))
    .execute({ actionId, ...(body.evidenceRef ? { evidenceRef: body.evidenceRef } : {}) }, resolution.commandContext);
