import { MarkLearningStaleService } from "@artist-os/core";
...
export async function POST(request: Request, context: { params: Promise<{ learningId: string }> }) {
  ...
  const result = await new MarkLearningStaleService(new PgLearningWriter(getDatabaseRuntime().db)).execute({ learningId, ...(body.rationale ? { rationale: body.rationale } : {}) }, resolution.commandContext);
