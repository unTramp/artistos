import { CreateCandidateKnowledgeService, type CreateCandidateKnowledgeCommand } from "@artist-os/core";
...
export async function POST(request: Request) {
  ...
  const body = await readJson<CreateCandidateKnowledgeCommand>(request, resolution.traceId);
  if (body instanceof Response) return body;

  const result = await new CreateCandidateKnowledgeService(new PgKnowledgeWriter(getDatabaseRuntime().db))
