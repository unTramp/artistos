...
export default async function WeeklyReviewsPage() {
  ...
  if (!actorContext) {
    return <AppShell activeId="memory"><section className="empty-state"><p className="eyebrow">AUTH REQUIRED</p><h1>Weekly Review</h1><p>Sign in before Artist OS can review private career context.</p><a className="inline-link" href="/auth">Sign in →</a></section></AppShell>;
  }
  if (!actorContext.artistId) {
    return <AppShell activeId="memory" sessionEmail={actorContext.user.email}><section className="empty-state"><p className="eyebrow">WORKSPACE REQUIRED</p><h1>Weekly Review</h1><p>Create the artist workspace first.</p><a className="inline-link" href="/">Open Today →</a></section></AppShell>;
  }
