...
export function CurrentFocusEditor({ currentDate, current, locale }: { currentDate: string; current: CurrentFocus | null; locale: Locale }) {
  const router = useRouter();
  const copy = getUiCopy(locale).focus;
  ...
  const createFocus = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setMessage(copy.createTryAgain);
    } finally {
      setPending(false);
    }
  };
  ...
  const completeFocus = async () => {
    ...
    try {
      ...
      setMessage(copy.completeTryAgain);
    } finally {
      setPending(false);
    }
  };
  ...
  return (
    <section className={`today-focus-empty${open ? " is-open" : ""}`} id="current-focus">
      {!open ? (
        <>
          <div>
            <span className="signal-label">{copy.current}</span>
            <strong>{copy.noPrimary}</strong>
            <p>{copy.noPrimaryBody}</p>
          </div>
          <button className="focus-set-button" type="button" onClick={() => setOpen(true)}>{copy.setCurrent}</button>
        </>
      ) : (
        <form className="focus-editor-form" onSubmit={createFocus}>
          <div className="focus-editor-heading">
            <div>
              <span className="signal-label">{copy.setHeading}</span>
              <strong>{copy.whatMatters}</strong>
              <p>{copy.whatMattersBody}</p>
            </div>
          ...
          <div className="focus-editor-footer">
            ...
            <button className="primary-action" type="submit" disabled={pending}>{pending ? copy.saving : copy.set}</button>
          </div>
        </form>
      )}
    </section>
