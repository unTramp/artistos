...
test("capture Russian Today after EN/RU localization", async ({ page }) => {
  ...
  const decision = await post(page, "/api/v1/decisions", {
    title: "Сделать клип с живым исполнением главным форматом",
    decision: "Для следующей серии контента ставим клип с живым исполнением в центр.",
  ...
  const focus = await post(page, "/api/v1/objectives", {
    ...
    statement: "Собрать сильный клип с живым исполнением и довести релизный контекст до готового действия.",
    periodStart: today,
  ...
  const actionTitle = "Подготовить клип с живым исполнением для Trastevere";
  const action = await post(page, "/api/v1/actions", {
    sourceDomain: "INTELLIGENCE",
    sourceEntityType: "Decision",
    sourceEntityId: decisionId,
    title: actionTitle,
    description: "Собрать сильное начало, структуру кадра и подготовить материал к съёмке.",
