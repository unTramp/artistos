...
test("validates a Learning, surfaces it on Today and uses it in Decision lineage", async ({ page }) => {
  ...
  await expect(learningRef.getByRole("link", { name: statement, exact: false })).toHaveAttribute("href", `/learnings#learning-${(await page.evaluate(async () => {
    ...
    return body.data.learnings.find((learning) => learning.statement === statement)?.id ?? "";
  }))}`);
});
