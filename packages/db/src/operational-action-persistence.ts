...
import { OperationalActionPersistenceError, type FoundationEvidence } from "@artist-os/core";
import type { Stage0Database } from "./runtime";
import { auditEvents, idempotencyRecords, outboxEvents } from "./schema";

export type OperationalActionTx = Parameters<Parameters<Stage0Database["transaction"]>[0]>[0];

const actorFields = (evidence: FoundationEvidence) => evidence.actorId ? { actorId: evidence.actorId } : {};

export const runOperationalActionIdempotent = async <T>(
  tx: OperationalActionTx,
  artistId: string,
  ...
  const scope = `OperationalAction:${commandName}:${artistId}:${actorScope}`;
  ...
  if (!claim) {
    ...
    throw new OperationalActionPersistenceError("IDEMPOTENCY_IN_PROGRESS");
  }
...
export const writeOperationalActionEvidence = async (
  tx: OperationalActionTx,
  input: {
    artistId: string;
    actionId: string;
  ...
  await tx.insert(outboxEvents).values({
    ...
    aggregateType: "OperationalAction",
