...
- **MASTER references:** §220, §192–202, §224–225
- **Domain:** 18_platforms_integrations
- **Feature slug:** `platform-accounts`
- **Requirement prefix:** `PLT-ACC`

## 2. Purpose
Represent the artist’s account type, restrictions and current capabilities per platform so downstream workflows know what is actually possible.
...
- music rights mode
- capability links
- advertising extension placeholders
...
## 14. Business rules
- `PLT-ACC-001` account profile is operational metadata, not artist identity
- `PLT-ACC-002` unknown restrictions remain unknown
- `PLT-ACC-003` account capability derived from registry/current verification
- `PLT-ACC-004` no credentials in entity
- `PLT-ACC-005` advertising capabilities remain extension-only
- `PLT-ACC-006` account changes invalidate dependent readiness checks
...
- `PLT-ACC-AC01` Manual/CSV fallback remains available.
- `PLT-ACC-AC02` Stale rule is visibly stale.
- `PLT-ACC-AC03` Secrets never reach frontend/logs.
- `PLT-ACC-AC04` Schema drift does not silently corrupt imports.
