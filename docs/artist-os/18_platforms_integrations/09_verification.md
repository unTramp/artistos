# Social Handle & Verification Tracking

- **Status:** REVIEW COMPLETE
- **MASTER references:** §380–382
- **Domain:** 18_platforms_integrations
- **Feature slug:** `verification`
- **Requirement prefix:** `PLT-VER`

## 2. Purpose
Track desired/current handles and verification state with evidence without embedding outdated press-count folklore.
...
## 4. Scope
### In scope
- SocialHandleRecord status
- profile URL/check date
- verification evidence
- availability checks/manual state
...
## 14. Business rules
- `PLT-VER-001` handle status enum respected
- `PLT-VER-002` availability claims timestamped
- `PLT-VER-003` verification criteria source-backed
- `PLT-VER-004` fixed number of press articles never hardcoded
- `PLT-VER-005` unknown status valid
- `PLT-VER-006` no automated squatting/impersonation actions
- `PLT-VER-007` disputes stored operationally not legal adjudication
...
- `PLT-VER-AC01` Manual/CSV fallback remains available.
- `PLT-VER-AC02` Stale rule is visibly stale.
- `PLT-VER-AC03` Secrets never reach frontend/logs.
- `PLT-VER-AC04` Schema drift does not silently corrupt imports.
