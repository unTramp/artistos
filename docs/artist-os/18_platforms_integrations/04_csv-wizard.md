...
- **MASTER references:** §371–373, §377
- **Domain:** 18_platforms_integrations
- **Feature slug:** `csv-wizard`
- **Requirement prefix:** `PLT-CSV`

## 2. Purpose
Provide Upload → Detect → Map → Preview → Validate → Import for heterogeneous platform and DSP exports.
...
## 14. Business rules
- `PLT-CSV-001` wizard follows MASTER step order
- `PLT-CSV-002` preview required before irreversible import
- `PLT-CSV-003` ambiguous mapping surfaced
- `PLT-CSV-004` missing columns not fabricated
- `PLT-CSV-005` large imports use jobs
- `PLT-CSV-006` source file/hash provenance stored
- `PLT-CSV-007` import supports cancellation before commit where feasible
...
- `PLT-CSV-AC01` Manual/CSV fallback remains available.
- `PLT-CSV-AC02` Stale rule is visibly stale.
- `PLT-CSV-AC03` Secrets never reach frontend/logs.
- `PLT-CSV-AC04` Schema drift does not silently corrupt imports.
