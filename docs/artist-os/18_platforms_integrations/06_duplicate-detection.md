...
- **MASTER references:** §373, §288–289
- **Domain:** 18_platforms_integrations
- **Feature slug:** `duplicate-detection`
- **Requirement prefix:** `PLT-DUP`

## 2. Purpose
Prevent repeated imports from double-counting observations while allowing intentional reposts/corrections.
...
- platformContentId/snapshotDate matching
- hash/source IDs
- conflict resolution
...
## 14. Business rules
- `PLT-DUP-001` duplicate detection must distinguish import duplicate from intentional content duplicate
- `PLT-DUP-002` conflicting records require review/version
- `PLT-DUP-003` never silently sum duplicated snapshots
- `PLT-DUP-004` idempotent import jobs
- `PLT-DUP-005` raw source identity retained
- `PLT-DUP-006` manual override audited
...
- `PLT-DUP-AC01` Manual/CSV fallback remains available.
- `PLT-DUP-AC02` Stale rule is visibly stale.
- `PLT-DUP-AC03` Secrets never reach frontend/logs.
- `PLT-DUP-AC04` Schema drift does not silently corrupt imports.
