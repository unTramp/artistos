...
- **MASTER references:** §379–381
- **Domain:** 18_platforms_integrations
- **Feature slug:** `presence-registry`
- **Requirement prefix:** `PLT-PRS`

## 2. Purpose
Track canonical artist name, domains, handles, profiles, trademarks and verification records as operational presence metadata.
...
## 14. Business rules
- `PLT-PRS-001` presence registry not legal advice
- `PLT-PRS-002` name/handle state preserves verification date
- `PLT-PRS-003` duplicate profile mappings surfaced
- `PLT-PRS-004` trademark tracking operational only
- `PLT-PRS-005` changes audited
- `PLT-PRS-006` canonical identity does not auto-rename platform accounts
...
- `PLT-PRS-AC01` Manual/CSV fallback remains available.
- `PLT-PRS-AC02` Stale rule is visibly stale.
- `PLT-PRS-AC03` Secrets never reach frontend/logs.
- `PLT-PRS-AC04` Schema drift does not silently corrupt imports.
