...
- **MASTER references:** §217–222, §227–230
- **Domain:** 18_platforms_integrations
- **Feature slug:** `capability-registry`
- **Requirement prefix:** `PLT-CAP`

## 2. Purpose
Keep platform capabilities, eligibility requirements and freshness as data-backed knowledge instead of permanent hardcoded assumptions.
...
## 14. Business rules
- `PLT-CAP-001` capability MUST carry source/lastVerifiedAt
- `PLT-CAP-002` hardcoded follower thresholds prohibited
- `PLT-CAP-003` stale/unverified rule not shown as current fact
- `PLT-CAP-004` region/account type respected
- `PLT-CAP-005` capability absence from data not equal to unsupported
- `PLT-CAP-006` official source preferred for operational rules
- `PLT-CAP-007` historical capability changes preserved
...
- `PLT-CAP-AC01` Manual/CSV fallback remains available.
- `PLT-CAP-AC02` Stale rule is visibly stale.
- `PLT-CAP-AC03` Secrets never reach frontend/logs.
- `PLT-CAP-AC04` Schema drift does not silently corrupt imports.
