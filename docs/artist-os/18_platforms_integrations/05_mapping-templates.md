...
- **MASTER references:** §372–373, §287–290
- **Domain:** 18_platforms_integrations
- **Feature slug:** `mapping-templates`
- **Requirement prefix:** `PLT-MAP`

## 2. Purpose
Reuse source-specific mapping knowledge without coupling the system to a single export schema.
...
## 14. Business rules
- `PLT-MAP-001` template scoped by source/schema version
- `PLT-MAP-002` schema drift detected
- `PLT-MAP-003` template change versioned
- `PLT-MAP-004` old imports remain reproducible
- `PLT-MAP-005` AI mapping suggestion never silently overwrites stable template
- `PLT-MAP-006` unit transforms explicit
...
- `PLT-MAP-AC01` Manual/CSV fallback remains available.
- `PLT-MAP-AC02` Stale rule is visibly stale.
- `PLT-MAP-AC03` Secrets never reach frontend/logs.
- `PLT-MAP-AC04` Schema drift does not silently corrupt imports.
