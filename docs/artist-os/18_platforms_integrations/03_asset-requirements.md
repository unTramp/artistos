...
- **MASTER references:** §221–222, §199–201
- **Domain:** 18_platforms_integrations
- **Feature slug:** `asset-requirements`
- **Requirement prefix:** `PLT-AST`

## 2. Purpose
Store current source-backed platform size/format/duration restrictions so adaptation and publishing validation do not rely on stale constants.
...
## 4. Scope
### In scope
- dimensions/formats/duration/metadata restrictions
- source/freshness
- platform/account/region scope
...
## 14. Business rules
- `PLT-AST-001` requirements must be source-backed
- `PLT-AST-002` stale requirement warns before blocking
- `PLT-AST-003` no universal aspect-ratio myth hardcoded
- `PLT-AST-004` raw provider requirement retained
- `PLT-AST-005` changes do not rewrite historical publication validity
- `PLT-AST-006` manual override requires rationale
...
- `PLT-AST-AC01` Manual/CSV fallback remains available.
- `PLT-AST-AC02` Stale rule is visibly stale.
- `PLT-AST-AC03` Secrets never reach frontend/logs.
- `PLT-AST-AC04` Schema drift does not silently corrupt imports.
