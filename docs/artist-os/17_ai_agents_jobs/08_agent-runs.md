...
- **MASTER references:** §334, §126, §391–393
- **Domain:** 17_ai_agents_jobs
- **Feature slug:** `agent-runs`
- **Requirement prefix:** `AI-RUN`

## 2. Purpose
Persist operational metadata for every AI run so cost, latency, configuration and context can be debugged and evaluated.
...
## 14. Business rules
- `AI-RUN-001` run links configuration version
- `AI-RUN-002` token/cost/latency recorded where provider gives them
- `AI-RUN-003` no secrets in run payload
- `AI-RUN-004` context metadata observable
- `AI-RUN-005` failed runs preserved
- `AI-RUN-006` user feedback/rejection linked
- `AI-RUN-007` chain-of-thought not stored/exposed as product requirement
...
- `AI-RUN-AC01` The feature follows its MASTER role/permission boundary.
- `AI-RUN-AC02` Provider failure leaves a usable manual/deterministic product path where applicable.
- `AI-RUN-AC03` Every run is tied to configuration/context metadata.
- `AI-RUN-AC04` No publish/delete/spend occurs without explicit approval.
