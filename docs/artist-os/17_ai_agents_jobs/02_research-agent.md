...
- **MASTER references:** §325, §28–35, §330
- **Domain:** 17_ai_agents_jobs
- **Feature slug:** `research-agent`
- **Requirement prefix:** `AI-RSH`

## 2. Purpose
Produce source-grounded current research with authority/freshness metadata and structured application guidance.
...
- ResearchClaim extraction
- source authority/freshness
- structured Finding/Evidence/Source/Date/Authority/Confidence/Application
...
## 14. Business rules
- `AI-RSH-001` must prefer primary source for platform rules
- `AI-RSH-002` must separate source claim from verification
- `AI-RSH-003` cannot promote practitioner advice to fact
- `AI-RSH-004` cannot publish or alter platform settings
- `AI-RSH-005` stale source must be labelled
- `AI-RSH-006` output sources/provenance required
- `AI-RSH-007` unsupported conclusion returns uncertainty
...
- `AI-RSH-AC01` The feature follows its MASTER role/permission boundary.
- `AI-RSH-AC02` Provider failure leaves a usable manual/deterministic product path where applicable.
- `AI-RSH-AC03` Every run is tied to configuration/context metadata.
- `AI-RSH-AC04` No publish/delete/spend occurs without explicit approval.
