...
- **MASTER references:** §322–324, §330, §338, §347
- **Domain:** 17_ai_agents_jobs
- **Feature slug:** `orchestrator`
- **Requirement prefix:** `AI-ORC`

## 2. Purpose
Route each AI task to the smallest suitable workflow, context, tools and approval policy rather than creating an uncontrolled agent swarm.
...
## 14. Business rules
- `AI-ORC-001` MVP physical agents remain limited
- `AI-ORC-002` Orchestrator MUST not publish/delete/spend
- `AI-ORC-003` tool permissions must be explicit
- `AI-ORC-004` unknown task may fall back to safe draft/manual path
- `AI-ORC-005` budget/time limits enforced
- `AI-ORC-006` context comes from Context Assembler
- `AI-ORC-007` routing/config version recorded
- `AI-ORC-008` no agent swarm proliferation without product need
...
- `AI-ORC-AC01` The feature follows its MASTER role/permission boundary.
- `AI-ORC-AC02` Provider failure leaves a usable manual/deterministic product path where applicable.
- `AI-ORC-AC03` Every run is tied to configuration/context metadata.
- `AI-ORC-AC04` No publish/delete/spend occurs without explicit approval.
