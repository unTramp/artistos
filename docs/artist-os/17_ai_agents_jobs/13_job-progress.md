...
- **MASTER references:** §349–353, §347
- **Domain:** 17_ai_agents_jobs
- **Feature slug:** `job-progress`
- **Requirement prefix:** `AI-JPR`

## 2. Purpose
Give understandable progress and recovery for long-running work without blocking normal application navigation.
...
## 4. Scope
### In scope
- Queued/Researching/Analyzing/Generating/Quality Check/Done/Failed
- cancel/retry
...
## 14. Business rules
- `AI-JPR-001` progress labels use MASTER vocabulary
- `AI-JPR-002` core UI loads independently of job completion
- `AI-JPR-003` failed job preserves diagnostic/action
- `AI-JPR-004` retry not offered for nonretryable errors
- `AI-JPR-005` uncertain external side effects require verification before retry
- `AI-JPR-006` multiple jobs should not freeze navigation
- `AI-JPR-007` completion links to resulting entity
...
- `AI-JPR-AC01` The feature follows its MASTER role/permission boundary.
- `AI-JPR-AC02` Provider failure leaves a usable manual/deterministic product path where applicable.
- `AI-JPR-AC03` Every run is tied to configuration/context metadata.
- `AI-JPR-AC04` No publish/delete/spend occurs without explicit approval.
