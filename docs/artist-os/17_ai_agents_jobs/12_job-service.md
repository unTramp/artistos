...
- **MASTER references:** §348–352
- **Domain:** 17_ai_agents_jobs
- **Feature slug:** `job-service`
- **Requirement prefix:** `AI-JOB`

## 2. Purpose
Run long AI/media/import/research operations safely with replaceable queue, idempotency and controlled retry.
...
## 14. Business rules
- `AI-JOB-001` every job idempotency key
- `AI-JOB-002` at-least-once safe
- `AI-JOB-003` queue replaceable/Postgres acceptable MVP
- `AI-JOB-004` retry only retryable errors
- `AI-JOB-005` dead-letter state visible
- `AI-JOB-006` cancel semantics explicit
- `AI-JOB-007` job payload avoids secrets
- `AI-JOB-008` domain not coupled to queue vendor
...
- `AI-JOB-AC01` The feature follows its MASTER role/permission boundary.
- `AI-JOB-AC02` Provider failure leaves a usable manual/deterministic product path where applicable.
- `AI-JOB-AC03` Every run is tied to configuration/context metadata.
- `AI-JOB-AC04` No publish/delete/spend occurs without explicit approval.
