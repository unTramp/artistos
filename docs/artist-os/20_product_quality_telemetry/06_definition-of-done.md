...
- **MASTER references:** §402, §463–464
- **Domain:** 20_product_quality_telemetry
- **Feature slug:** `definition-of-done`
- **Requirement prefix:** `QTY-DOD`

## 2. Purpose
Make completion mean verified working product behavior, not merely code that compiles.
...
## 14. Business rules
- `QTY-DOD-001` typecheck required
- `QTY-DOD-002` lint required
- `QTY-DOD-003` relevant tests required
- `QTY-DOD-004` manual smoke required
- `QTY-DOD-005` no console errors
- `QTY-DOD-006` no broken routes
- `QTY-DOD-007` known limitations/technical debt reported
- `QTY-DOD-008` architecture deviations documented
- `QTY-DOD-009` risky migration has backup/rollback
- `QTY-DOD-010` feature not done if acceptance criteria unverified
...
- `QTY-DOD-AC01` Metric/test result is reproducible enough to inspect.
- `QTY-DOD-AC02` Small samples are labelled.
- `QTY-DOD-AC03` DoD cannot pass with broken routes/console errors.
- `QTY-DOD-AC04` AI quality and cost are not collapsed into one score.
