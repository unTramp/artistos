...
- **MASTER references:** §396–401
- **Domain:** 20_product_quality_telemetry
- **Feature slug:** `testing`
- **Requirement prefix:** `QTY-TST`

## 2. Purpose
Define the required unit, integration, agent/eval and E2E coverage for core Artist OS behavior and safety boundaries.
...
## 4. Scope
### In scope
- unit state/metrics/context/learning/rights/import tests
- integration repos/vector/jobs/transcription/adapters/publishing/storage
- agent permission/grounding tests
- identity evals
- E2E Core/Release
...
## 14. Business rules
- `QTY-TST-001` unit tests cover state machines/metrics/context/learning/confidence/audio/segments/lineage/rights/imports/identity
- `QTY-TST-002` integration tests cover external boundaries
- `QTY-TST-003` Research agent cannot publish test
- `QTY-TST-004` Analytics cannot invent metrics test
- `QTY-TST-005` Context excludes unrelated Song Brain test
- `QTY-TST-006` Guard respects mystique/rights/deviation test
- `QTY-TST-007` E2E Core and Release paths required before major milestone
...
- `QTY-TST-AC01` Metric/test result is reproducible enough to inspect.
- `QTY-TST-AC02` Small samples are labelled.
- `QTY-TST-AC03` DoD cannot pass with broken routes/console errors.
- `QTY-TST-AC04` AI quality and cost are not collapsed into one score.
