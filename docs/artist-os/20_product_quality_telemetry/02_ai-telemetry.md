...
- **MASTER references:** §393, §331–338
- **Domain:** 20_product_quality_telemetry
- **Feature slug:** `ai-telemetry`
- **Requirement prefix:** `QTY-AI`

## 2. Purpose
Monitor AI usefulness, cost, latency and rejection patterns so configs improve based on evidence rather than anecdote.
...
## 14. Business rules
- `QTY-AI-001` cost/latency/quality reported separately
- `QTY-AI-002` acceptance not sole quality metric
- `QTY-AI-003` edit distance definition consistent
- `QTY-AI-004` rejection reasons structured
- `QTY-AI-005` guard override false-positive review
- `QTY-AI-006` provider/model comparisons use same eval/task context
- `QTY-AI-007` telemetry does not expose prompts/private corpus unnecessarily
...
- `QTY-AI-AC01` Metric/test result is reproducible enough to inspect.
- `QTY-AI-AC02` Small samples are labelled.
- `QTY-AI-AC03` DoD cannot pass with broken routes/console errors.
- `QTY-AI-AC04` AI quality and cost are not collapsed into one score.
