...
- **MASTER references:** §394, §170–174
- **Domain:** 20_product_quality_telemetry
- **Feature slug:** `smart-ingest-kpi`
- **Requirement prefix:** `QTY-SMI`

## 2. Purpose
Measure actual manual organization time before/after Smart Ingest instead of claiming an arbitrary efficiency percentage.
...
## 14. Business rules
- `QTY-SMI-001` no arbitrary percent improvement claim
- `QTY-SMI-002` same/similar batch complexity noted
- `QTY-SMI-003` corrections included in assisted time
- `QTY-SMI-004` rejected suggestions counted
- `QTY-SMI-005` small samples qualified
- `QTY-SMI-006` KPI never rewards aggressive wrong auto-mapping
...
- `QTY-SMI-AC01` Metric/test result is reproducible enough to inspect.
- `QTY-SMI-AC02` Small samples are labelled.
- `QTY-SMI-AC03` DoD cannot pass with broken routes/console errors.
- `QTY-SMI-AC04` AI quality and cost are not collapsed into one score.
