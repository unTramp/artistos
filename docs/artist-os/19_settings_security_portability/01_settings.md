...
- **MASTER references:** §354–370, §383–391
- **Domain:** 19_settings_security_portability
- **Feature slug:** `settings`
- **Requirement prefix:** `SEC-SET`

## 2. Purpose
Centralize non-domain configuration, integrations, defaults and data-management controls without turning Settings into hidden business logic.
...
- integration links
- timezone/defaults
- data/export/security entry points
...
## 14. Business rules
- `SEC-SET-001` settings must not duplicate domain truth
- `SEC-SET-002` timezone/defaults visible and versionable where material
- `SEC-SET-003` dangerous actions separated/confirmed
- `SEC-SET-004` AI provider config not expose secrets
- `SEC-SET-005` single-artist first; no premature enterprise roles
- `SEC-SET-006` settings changes that affect behavior audited
...
- `SEC-SET-AC01` Sensitive material is not exposed by default.
- `SEC-SET-AC02` Material actions are auditable.
- `SEC-SET-AC03` Secrets are never returned in plaintext UI/logs.
- `SEC-SET-AC04` Portability works without provider lock-in.
