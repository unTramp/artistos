...
- **MASTER references:** §386–387, §214, §271
- **Domain:** 19_settings_security_portability
- **Feature slug:** `privacy`
- **Requirement prefix:** `SEC-PRV`

## 2. Purpose
Protect Internal Canon and minimize fan/person data by default across integrations and exports.
...
## 4. Scope
### In scope
- internal canon classification
- PII minimization
- external sharing boundary
- aggregate fan data default
...
## 14. Business rules
- `SEC-PRV-001` internal canon not sent externally without necessity
- `SEC-PRV-002` fan PII not stored by default
- `SEC-PRV-003` person-level scope requires explicit feature/privacy decision
- `SEC-PRV-004` exports respect private/public modes
- `SEC-PRV-005` AI context applies access filters
- `SEC-PRV-006` webhook payloads with unexpected PII redacted/discarded
- `SEC-PRV-007` privacy defaults favor minimization
...
- `SEC-PRV-AC01` Sensitive material is not exposed by default.
- `SEC-PRV-AC02` Material actions are auditable.
- `SEC-PRV-AC03` Secrets are never returned in plaintext UI/logs.
- `SEC-PRV-AC04` Portability works without provider lock-in.
