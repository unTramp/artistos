...
- **MASTER references:** §388, §316–318
- **Domain:** 19_settings_security_portability
- **Feature slug:** `audit-events`
- **Requirement prefix:** `SEC-AUD`

## 2. Purpose
Record material knowledge, identity, approval, publishing, config and rights changes so actions and decisions are reconstructable.
...
## 14. Business rules
- `SEC-AUD-001` identity/knowledge/decision/approval/publish/rule promotion/config/rights override audited
- `SEC-AUD-002` audit history append-only in normal UX
- `SEC-AUD-003` sensitive values redacted
- `SEC-AUD-004` actor/source distinguished user/agent/provider
- `SEC-AUD-005` reversal links prior event
- `SEC-AUD-006` audit does not replace domain history
...
- `SEC-AUD-AC01` Sensitive material is not exposed by default.
- `SEC-AUD-AC02` Material actions are auditable.
- `SEC-AUD-AC03` Secrets are never returned in plaintext UI/logs.
- `SEC-AUD-AC04` Portability works without provider lock-in.
