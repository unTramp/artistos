# Knowledge & Portability Exports

- **Status:** REVIEW COMPLETE
- **MASTER references:** §389–390
- **Domain:** 19_settings_security_portability
- **Feature slug:** `exports`
- **Requirement prefix:** `SEC-EXP`

## 2. Purpose
Let the artist export critical knowledge and history in portable JSON/Markdown so product value is not trapped in one vendor.
...
## 14. Business rules
- `SEC-EXP-001` JSON/Markdown supported
- `SEC-EXP-002` export preserves provenance/version where practical
- `SEC-EXP-003` private data warning before export
- `SEC-EXP-004` secrets/tokens excluded
- `SEC-EXP-005` large exports job-based
- `SEC-EXP-006` partial export clearly reports exclusions/errors
- `SEC-EXP-007` portable IDs/references stable enough to interpret
...
- `SEC-EXP-AC01` Sensitive material is not exposed by default.
- `SEC-EXP-AC02` Material actions are auditable.
- `SEC-EXP-AC03` Secrets are never returned in plaintext UI/logs.
- `SEC-EXP-AC04` Portability works without provider lock-in.
