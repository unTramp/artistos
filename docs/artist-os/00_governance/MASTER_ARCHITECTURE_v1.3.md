...
**Статус Advertising/Publicity:** research boundary; не входит в нормативное ядро v1.3, кроме общих extension points.

> v1.3 заменяет MASTER-ТЗ v1.1 как рабочий архитектурный документ. v1.1 сохраняется только как архив. Концепции Artist Identity Engine из промежуточной v1.2 включены в v1.3 как часть основной архитектуры.

# ЧАСТЬ I. VERSION SCOPE & ARCHITECTURAL FREEZE

## 1. Что фиксирует v1.3

v1.3 фиксирует Organic & Operational Core Artist OS: идентичность артиста, narrative, песни, контент, производство, assets, distribution, DSP, organic growth, market intelligence, fan value/business и общий analytics/learning контур.

## 2. Что сознательно не смешивается с v1.3

Advertising Intelligence и Publicity/PR исследуются следующими отдельными потоками. v1.3 содержит только интерфейсы расширения Campaign/Attribution/Research для будущего подключения этих доменов.
...
Если более старые заметки, курсы, prompt'ы или implementation decisions конфликтуют с этим MASTER-ТЗ, приоритет имеет v1.3, пока человек явно не утвердит изменение.
...
Campaign может связывать song/release, identity version, era, narrative tracks, content units, publications, DSP actions, website experience, growth experiments и offers.
...
## 156. Monthly Objective first

Calendar строится от Monthly Objective, current Campaign, Narrative Mix, Evergreen requirements, Production Capacity, Existing Assets и active Experiments.
...
Поддержать parentAssetId + derivationType TRIM/CROP/COLOR_GRADE/CAPTIONED/MIXED/EXPORT/REMIX.
...
Собственный CRM не входит в v1.3. OS хранит provider/form/campaign и агрегированные conversion metrics; PII остаётся у специализированного provider, если не подключён явный CRM scope.
...
Не строить taxes, payroll, depreciation, double-entry bookkeeping в v1.3.
...
```text
Create Artist → Identity → Song → Angle → Approve → Execution → Shoot → Smart Ingest → Select Take → Publish → Import Metrics → Insight → Hypothesis → Experiment → Learning → Decision → Next Strategy
```

## 401. E2E Release

```text
Create Release → DSP Readiness → Editorial Opportunity → Content/Link plan → Launch → Post-launch metrics → Source analysis → Learning
```
...
Dashboard, Artist Brain, Songs, Content Factory, Pipeline, Calendar, Shoot Planner, On-Set, Basic Assets, Smart Ingest basic, manual Publication, CSV Metrics, Analytics, Insights, Experiments, Learnings, Decision Log.
...
Song Segments, AudioUsage, DSP Profile Readiness, Release Readiness, manual DSPOpportunity tracking, source-of-streams CSV/manual where available.
...
Upload, metadata, transcription, device timestamps/offsets, assisted linking, take grouping, rights status.
...
Artist, Song, Campaign, Content, Asset, Publication, Metrics, Experiment, Learning, Decision + identity root/version entities.
...
Transcription/Voice, Shoot Planner, On-Set, Smart Ingest, rights, production capability, atomization basics.

## 429. Stage 13 — Planning & Content Intelligence

Song Segments, Hooks, Narrative/Content modes, Evergreen, Recurring Series, Novelty/Fatigue.

## 430. Stage 14 — Distribution Foundation

Profile Readiness, Publication adaptations, Link Routing, Owned Media model, Platform Capability Registry.

## 431. Stage 15 — DSP Intelligence

DSP profiles, Release Readiness, opportunities, source-of-streams imports, playlist research, revenue imports.
...
# ЧАСТЬ XXXV. RESEARCH BOUNDARY AFTER v1.3
...
Это является целевой архитектурой **Artist OS v1.3**.
